...
$$
, where y is the count of bike trips, $x_i$ is the ith explanatory variable, $\epsilon$ is th erroe term, $r$ is the region index, and $t$ is the time index. We can implement the model in R using plm package. The pseudo methodology would be:
start with the panel data, df $\leftarrow$ convert to a pdata.frame object using \emph{pdata.frame(df, c("region","year"))} $\leftarrow$ run the pooled spatio-temporal model using \emph{df.pool = plm(gy~gx, data=df, model="pooling")}. Finally use \emph{summary(df.pool)} to see what spatial factors explain the changes in teh count of bike trips for each cell between 2019 and 2020. 