...
library(pdxTrees)
tree  <- makeIcon(
  iconUrl = "http://clipart-library.com/new_gallery/6-60593_clean-energy-tree-icon-copy-green-tree-icon.png",
  iconWidth = 15, iconHeight = 25)
content <- paste("<b>", Rhodies_garden$common_name, ",", Rhodies_garden$scientific_name,
                "</b></br>", "Condition:", Rhodies_garden$condition, "</br>", "Native:", Rhodies_garden$native, "</br>", "Mature Size:", Rhodies_garden$mature_size)
pdxTrees <- as.data.frame(pdxTrees)
Rhodies_garden <- pdxTrees %>% filter(park=="Crystal Springs Rhododendron Garden")
Rhodies_garden %>%
leaflet(options = leafletOptions(minZoom = 15, maxZoom = 20)) %>%
   setView(lng = -122.6355, lat = 45.4799,
          zoom = 15) %>%     
  addTiles() %>%
  addMarkers(lng = ~longitude , lat = ~latitude, 
                   icon = tree, popup = content) %>%
  addMiniMap()
```

```{r}
pal <- colorFactor(palette =c("Yellow", "Blue", "Red"), domain=Rhodies_garden$mature_size)
# mapping based on type
Rhodies_garden %>%
leaflet(options = leafletOptions(minZoom = 11, maxZoom = 20)) %>% 
  addProviderTiles(providers$Stamen.Watercolor) %>% 
   setView(lng = -122.6355, lat = 45.4799,
          zoom = 16) %>%  
  addCircleMarkers(~longitude, ~latitude, popup=Rhodies_garden$common_name, weight = 3, radius=4, color=~cof(mature_size), stroke = F, fillOpacity = 0.7) %>%
  addLegend("bottomright", pal=pal, values = ~mature_size,
    title = "Mature size of the tree",
    opacity = 1
  )
```
Key takeaways that we can learn from the maps:
*The map of the Rhododendron garden tells us about the exaxt shape of the park and the location of the trees.
*Surprisingly, most trees in the Rhodies Garden have large mature size. 
...
scatter(Rhodies_garden, dbh, tree_height, color = condition, alpha = 0.7)+
transition_states(tree_height, transition_length = 4) +
  labs(title = "dbh Vs. height of trees", subtitle = "grouped by tree condition") +  theme(plot.title = element_text(hjust = 0.5),plot.subtitle = element_text(hjust = 0.5))+
  shadow_mark()
```
Key takeaways that we can learn from your animated graph:
* Most of the trees in the Crystal Springs Rhododendrons garden are in fair condition.
* Height of the trees varies more as the diameter at breast height(dbh) increases. 
